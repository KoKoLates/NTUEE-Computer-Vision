...
def main():
    ...
    img_left_g = cv2.imread(os.path.join(args.dataset_path, args.image, 'img_left.png'), cv2.IMREAD_GRAYSCALE)
    img_right = cv2.imread(os.path.join(args.dataset_path, args.image, 'img_right.png'))
    img_right_g = cv2.imread(os.path.join(args.dataset_path, args.image, 'img_right.png'), cv2.IMREAD_GRAYSCALE)
